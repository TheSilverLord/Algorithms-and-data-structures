...
int main() {
	srand(time(0));
	setlocale(LC_ALL, "ru");
	Random_tree<int, int> tree;
	Random_tree<int, int>::Iterator* it = NULL;
	Random_tree<int, int>::r_Iterator* it1 = NULL;
	bool k = true;
	while (k) {
		menu();
		int _k;
		...
		case 1: {
			int val, key;
			std::cin >> key >> val;
			std::cout << tree.insert(key, val) << std::endl;;
			break; }
		case 2: {
		...
		case 19:
		{
			if (it == NULL) it = new Random_tree<int, int >::Iterator(&tree);
			it = tree.begin();
			break;
		}
		case 20:
		{
			if (it1 == NULL) it1 = new Random_tree<int, int >::r_Iterator(&tree);
			it1 = tree.rbegin();
			break;
		}
		case 21:
